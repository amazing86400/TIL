...
​	여기서 중요한 것은 `<div>`로 묶는 것과 부모 태그, 자식 태그 관계이다. `<ol>`태그는 이미 부모이지만 `<div>`에 의해 자식 태그가 되기도 한다. 그리고 이때 div의 id를 활용해 선택자로 활용하면 더 좋은 코드가 된다. '#grid ol'이라고 입력하면 더 구분하여 코드를 활용할 수 있게 된다.